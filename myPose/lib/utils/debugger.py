...
class Debugger(object):
  ...
  # mycode =================================================
  def add_ct_pt(self, pt, cat, conf=1 , show_txt = True,img_id = 'default'):
    ...
    cv2.circle(self.imgs[img_id], pt, 2, c, 2)
    
    return

    
  def add_object_frame(self, instrinsic, distortion, rotation_vec, translation_vec, scale = 0.5, img_id = 'default'):
   
    self.imgs[img_id] = cv2.drawFrameAxes(self.imgs[img_id], instrinsic, distortion, rotation_vec, translation_vec, scale)
  def draw_axis(self, rotation_vec, t, K, scale=0.1, dist=None, img_id = 'default'):
    """
    ...
    """
    self.imgs[img_id] = self.imgs[img_id].astype(np.float32)
    ...
    self.imgs[img_id] = cv2.line(self.imgs[img_id], tuple(axis_points[3].ravel()), tuple(axis_points[0].ravel()), (255, 0, 0), 3)
    self.imgs[img_id] = cv2.line(self.imgs[img_id], tuple(axis_points[3].ravel()), tuple(axis_points[1].ravel()), (0, 255, 0), 3)
    self.imgs[img_id] = cv2.line(self.imgs[img_id], tuple(axis_points[3].ravel()), tuple(axis_points[2].ravel()), (0, 0, 255), 3)
    
  

  def add_axis(self,axis_proj,cat, conf = 1, show_txt = True,img_id = 'default'):
    #print("axis_pts", pts)
    cv2.arrowedLine(self.imgs[img_id], axis_proj[0], axis_proj[1], [0,0,255],2)
    cv2.arrowedLine(self.imgs[img_id], axis_proj[0], axis_proj[2], [0,255,0],2)
    cv2.arrowedLine(self.imgs[img_id], axis_proj[0], axis_proj[3], [255,0,0],2)
  def add_3d_bbox(self, pts, cat, conf=1 , show_txt = True,img_id = 'default'):
    ...
    color = np.array([0,0,225], dtype = int).tolist()
    #print("color:",np.array(color, dtype = int).tolist())
    #print("key", key_point)
    for i in range(key_point.shape[0]):
      pt = key_point[i]
      cv2.circle(self.imgs[img_id], pt, 4, np.array([0,225,0], dtype = int).tolist(), 3)
      if(i>0):cv2.putText(self.imgs[img_id], str(i-1), pt, 1 , 1 , [0, 0 , 0], 1)
      #key_point[i] = pt
    #cv2.circle(self.imgs[img_id], pt, 2, c, 2)
    #key_point[0,:] = ct
    cv2.circle(self.imgs[img_id], key_point[0], 2, color, 2)
    

    cv2.line(self.imgs[img_id], key_point[1], key_point[2], color,2)
    cv2.line(self.imgs[img_id], key_point[1], key_point[5], color,2)
    cv2.line(self.imgs[img_id], key_point[6], key_point[2], color,2)
    cv2.line(self.imgs[img_id], key_point[5], key_point[6], color,2)

    cv2.line(self.imgs[img_id], key_point[3], key_point[7], color,2)
    cv2.line(self.imgs[img_id], key_point[4], key_point[8], color,2)
    cv2.line(self.imgs[img_id], key_point[3], key_point[4], color,2)
    cv2.line(self.imgs[img_id], key_point[7], key_point[8], color,2)

    cv2.line(self.imgs[img_id], key_point[1], key_point[3], color,2)
    cv2.line(self.imgs[img_id], key_point[4], key_point[2], color,2)
    cv2.line(self.imgs[img_id], key_point[8], key_point[6], color,2)
    cv2.line(self.imgs[img_id], key_point[7], key_point[5], color,2)
    return

  def add_single_grasp(self, pts, cat, conf=1 , show_txt = True,img_id = 'default', show_obj_ct = False, obj_ct = None):
    pts = np.array(pts, dtype = np.int32)
    cat = int(cat)
    c = self.colors[cat][0][0].tolist()
    if self.theme == 'white':
    ...
    key_point = np.array(pts).reshape((5,2)).astype(int)
    ...
    cv2.line(self.imgs[img_id], key_point[1], top_center, color,1)
    cv2.line(self.imgs[img_id], key_point[2], button_center, color,1)
    cv2.line(self.imgs[img_id], top_center, button_center, color,1)
    cv2.line(self.imgs[img_id], rect_center, right_center, color,1)
    
    if show_obj_ct and obj_ct.any():
      
      obj_ct = np.array(obj_ct, dtype = np.int32).reshape(2).astype(int)
      
      cv2.arrowedLine(self.imgs[img_id], key_point[0], obj_ct, color, 1)

  
    return

  def add_paired_grasp(self, pts, cat, conf=1 , show_txt = True,img_id = 'default', show_obj_ct = False, obj_ct = None):
    pts = np.array(pts, dtype = np.int32)
    cat = int(cat)
    c = self.colors[cat][0][0].tolist()
    if self.theme == 'white':
    ...
    cv2.line(self.imgs[img_id], key_point[1], top_center, color,2)
    cv2.line(self.imgs[img_id], key_point[2], button_center, color,2)
    cv2.line(self.imgs[img_id], top_center, button_center, color,2)
    cv2.line(self.imgs[img_id], rect_center, right_center, color,2)
    ...
    #cv2.line(self.imgs[img_id], key_point[5], top_center, color,2)
    #cv2.line(self.imgs[img_id], key_point[6], button_center, color,2)
    #v2.line(self.imgs[img_id], top_center, button_center, color,2)
    #cv2.line(self.imgs[img_id], rect_center, right_center, color,2)
    cv2.line(self.imgs[img_id], key_point[5], key_point[6], color,2)
    cv2.line(self.imgs[img_id], key_point[6], key_point[8], color,2)
    cv2.line(self.imgs[img_id], key_point[8], key_point[7], color,2)
    cv2.line(self.imgs[img_id], key_point[7], key_point[5], color,2)
    ...
    if show_obj_ct and obj_ct.any():
      ...
      cv2.arrowedLine(self.imgs[img_id], key_point[0], obj_ct, color, 1)
      cv2.arrowedLine(self.imgs[img_id], key_point[0], grasp0_center, color, 1)
      cv2.arrowedLine(self.imgs[img_id], key_point[0], grasp1_center, color, 1)
    return
...
coco_class_name = [
     'foup12', 'foup8', 'magazine', 'motorcycle', 'airplane',
