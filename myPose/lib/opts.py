...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--depth', type=bool, default = True)
    ...
    #mycode==========================================
    #pt.use_convGRU
    self.parser.add_argument('--use_convGRU', type = bool, default=False, 
                             help='use conv GRU of not')
    self.parser.add_argument('--use_3d_bounding_box_loss', type = bool, default=False)
    #================================================
    # input
    ...
    # train
    self.parser.add_argument('--lr', type=float, default=0.75e-4, 
    ...
    self.parser.add_argument('--trainval', action='store_true',
                             help='include validation in training and '
                                  'test on test set')
    self.parser.add_argument("--spilt_ratio", type=int, default = 4)
    ...
    #mycode===========================================================
    #沒有用 但刪掉很麻煩
    self.parser.add_argument('--eval_class', type=list , default = [0],
                             help='eval specific box classes ') #限制想要做eval的類別
    self.parser.add_argument('--train_class', type=list , default = [0],  #default = [i for i in range(60)],
                             help='train specific box classes ')
    #=================================================================
    # ddd
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if(opt.depth):
      opt.mean_rgbd, opt.std_rgbd = dataset.mean_rgbd, dataset.std_rgbd  #不知道為啥突然不會動
      #opt.mean_rgbd = [0.64326715, 0.64328622, 0.64328383, 0.7249166783727705]
      #opt.std_rgbd = [0.03159907, 0.03159791, 0.03159052, 0.06743566336832418]
    #print(dataset)
    opt.num_classes = dataset.num_classes
    opt.class_name = dataset.class_name
    #opt.train_class = dataset.train_class
    ...
    if opt.task == 'exdet':
      ...
      #mycode============================
        opt.heads.update({'kps': 16})
        opt.heads.update({'scale':3})
        #grasp
        opt.heads.update({'single_grasp_center_hm': 1,
                          'single_grasp_center_offset':2,

                          'single_grasp_ct_kps_dsp': 10,
                          'single_grasp_type':10,
                          #'single_grasp_kpt_mask':1, #沒有用到 但刪掉很麻煩
                          'single_grasp_width':1,
                          'single_graspct_objct_dsp':2
                          })
        opt.heads.update({'paired_grasp_center_hm': 1,
                          'paired_grasp_center_offset':2,

                          'paired_grasp_ct_kps_dsp': 18,
                          'paired_grasp_type':10,
                          #'paired_grasp_kpt_mask':1, #沒有用到 但刪掉很麻煩
                          'paired_grasp_width':2,
                          'paired_graspct_objct_dsp':2
                          })
      #==================================
  ...
  def init(self, args=''): # for demo and eval
    demo_class = [0,1]
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512],  
                ...
                'mean_rgbd':[0.64326715, 0.64328622, 0.64328383, 0.7249166783727705], 
                'std_rgbd':[0.03159907, 0.03159791, 0.03159052, 0.06743566336832418],
                'dataset': 'coco'},
